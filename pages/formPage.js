...
export default class FormElements {
  ...
  async currentButton(name) {
    return this.screen.getByRole('button', { name: name })
  }
  ...
  async expectTableTitlesVisibility(expectedArray, condition = true) {
    expectedArray.forEach(async (elem) => {
      condition === true ? expect(await this.currentTable()).toHaveTextContent(elem) : expect(await this.currentTable()).not.toHaveTextContent(elem)    
    })
  }
